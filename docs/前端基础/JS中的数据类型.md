...
> 为什么 undefined 和 null 明明是两种基础数据类型，但 undefined == null 返回的是 true ？
...
1. 由于前面所介绍的设计上的失误，获取 null 的数据类型会得到一个 object，这在开发上会带来一些未知的问题。
...
另外， **JS 中在调用函数的时候，通通都是值传递，而非引用传递**。
...
如果为简单值动态添加方法，则会报错 xxx is not a function.
